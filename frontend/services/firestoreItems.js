...
export const getCurrentUserItems = async (uid, lastDoc, pageSize) => {
  return paginateItems(lastDoc, pageSize, () =>
    where("giverid", "==", doc(firestore, "users", uid))
  );
};

export const getCurrentUserItemQueues = async (uid, lastDoc, pageSize) => {
  const { items: itemIdRefs, lastDoc: newLastDoc } = await paginateItems(
    lastDoc,
    pageSize,
  ...
  const { items } = await paginateItems(null, itemIds.length, () =>
